...
export function cleanupWaybackHeaders(
  ...
  for (const [key, value] of rawHeaders) {
    ...
    else if (key.toLowerCase() === "content-encoding" && !rawHeaders.some(([k]) => k.toLowerCase() === `${WAYBACK_ORIGINAL_HEADER_PREFIX}content-encoding`)) {
    ...
    else if (key.toLowerCase() === "content-type" && !rawHeaders.some(([k]) => k.toLowerCase() === `${WAYBACK_ORIGINAL_HEADER_PREFIX}content-type`)) {
