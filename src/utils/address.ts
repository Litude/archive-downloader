...
export function getWaybackCaptureBaseUrl(url: string): { timestamp: string, originalUrl: string } | null {
  ...
  const [, timestamp, originalUrl] = url.match(/https?:\/\/web\.archive\.org\/web\/(\d+)(?:[a-z]{2}_)?\/(.*)/) ?? [];
