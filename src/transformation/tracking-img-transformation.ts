...
function normalizeUrl(
  ...
  options: TrackingImageTransformationOptions,
) {
      ...
      if (part.startsWith("source=")) {
        ...
        if (isComplexUri && params) {
          ...
          if (options.queryParameters === "from-transformation") {
            ...
            const sortedEntries = Object.entries(queryParams).sort(([keyA], [keyB]) =>
              keyA.localeCompare(keyB),
            );
            const newParams = sortedEntries
              .filter(([_, value]) => value !== null)
              .map(([key, value]) => `${key}=${value}`)
              .join("&");
            const combinedResult = newParams
              ? `${basePart.toLowerCase()}?${newParams}`
...
function normalizeTrackingImageUrl(
  content: Buffer,
  queryParams: Record<string, string | null>,
  options: TrackingImageTransformationOptions,
): Buffer {
  ...
  const normalizedPrimary = trackingUrl
    ? normalizeUrl(trackingUrl, "&", queryParams, options)
    : null;
  const normalizedSecondary = secondaryTrackingUrl
    ? normalizeUrl(secondaryTrackingUrl, "&amp;", queryParams, options)
    : null;
...
function transformInputs(
  ...
  const normalizedContent = normalizeTrackingImageUrl(
    input.content,
    input.queryParams,
    transformationOptions as TrackingImageTransformationOptions,
  );
  return [
    {
...
function validateParameters(params: Record<string, any>): boolean {
  ...
  Object.entries(params).forEach(([key, value]) => {
    if (key === "queryParameters" && value !== "strip" && value !== "from-transformation") {
      throw new Error(
        `TrackingImageTransformation parameters incorrect: Invalid value for queryParameters: ${value}`,
      );
    } else if ((key === "path" || key === "defaultPath") && typeof value !== "string") {
      throw new Error(
        `TrackingImageTransformation parameters incorrect: Invalid value for ${key}: ${value}`,
      );
    }
  });
  return true;
