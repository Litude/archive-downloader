...
describe("normalizeTrackingImageUrl", () => {
  ...
  it("normalizes URL level host in tracking image URL", () => {
    ...
    expect(resultHtml).toContain(
      "&URI=%2flibrary%2ftoolbar%2f3.0%2fasp.aspx%3fmode%3dhead%26c%3d%2ffrance%2fjeux%2fjeux15.config%26h%3dwww%252Emicrosoft%252Ecom%26u%3d%252Ffrance%252Fjeux%252Fempires%252Fdefault%252Easp%26r%3d&GUID",
    );
    expect(resultHtml).not.toContain("r=");
  });
  ...
  it("does not modify tracking image URL when options URI has no subparts", () => {
    ...
    expect(resultHtml).toContain(
      "&URI=%2fbrasil%2fgames%2fage2%2fcaracteristicas.aspx&GUID=1F4FC18C-F71E-47FB-8FC9-612F8EE59C61&lc=pt-br",
    );
  });
  ...
  it("lowercases url path in tracking image as well", () => {
    ...
    expect(resultHtml).toContain("p=brasil_games_age2");
    expect(resultHtml).toContain(
      "&URI=%2fbrasil%2fgames%2fage2%2fdefault.aspx&GUID=1F4FC18C-F71E-47FB-8FC9-612F8EE59C61&lc=pt-br",
    );
  });
  ...
  it("normalises url with html encoded ampersands", () => {
    ...
    expect(resultHtml).toContain(
      "source=www&amp;TYPE=PV&amp;p=brasil_games_age2&amp;URI=%2fbrasil%2fgames%2fage2%2fdefault.aspx&amp;GUID=1F4FC18C-F71E-47FB-8FC9-612F8EE59C61&amp;lc=pt-br",
    );
  });
  ...
  it("normalises url with html encoded ampersands", () => {
    ...
    const html = createTestHtmlWithSecondaryUrl(trackingUrl1, trackingUrl2);
    const input = Buffer.from(html, "latin1");
    const result = TrackingImageNormalizer.normalize(
      input,
      {},
      {
    ...
    expect(resultHtml).toContain(
      "&URI=%2fgames%2fconquerors%2fcampaigns.aspx&GUID=1F4FC18C-F71E-47FB-8FC9-612F8EE59C61&lc=en-us",
    );
    expect(resultHtml).toContain(
      "source=www&amp;TYPE=PV&amp;p=games_conquerors&amp;URI=%2fgames%2fconquerors%2fcampaigns.aspx&amp;GUID=1F4FC18C-F71E-47FB-8FC9-612F8EE59C61&amp;lc=en-us",
    );
  });
  ...
  it("normalises url and adds query parameters from transformation", () => {
    ...
    expect(resultHtml).toContain("p=games_conquerors");
    expect(resultHtml).toContain(
      "&URI=%2fgames%2fconquerors%2fcivilizations.aspx%3ff1%3dyes%26f2%3dspanish&GUID=1F4FC18C-F71E-47FB-8FC9-612F8EE59C61&lc=en-us",
    );
    expect(resultHtml).toContain(
      "source=www&amp;TYPE=PV&amp;p=games_conquerors&amp;URI=%2fgames%2fconquerors%2fcivilizations.aspx%3ff1%3dyes%26f2%3dspanish&amp;GUID=1F4FC18C-F71E-47FB-8FC9-612F8EE59C61&amp;lc=en-us",
    );
  });
});
