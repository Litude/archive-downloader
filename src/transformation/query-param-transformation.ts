...
function queryParamTransformer(
  ...
  const options = validateOptions(inputOptions) ? inputOptions : (() => { throw new Error("Invalid options for QueryParamTransformer"); })();
      ...
      for (let i = matches.length - 1; i >= 0; i--) {
        ...
        if (rule.replacement) {
          ...
          for (let groupNum = 1; groupNum < indices.length; groupNum++) {
            ...
            replacement = replacement.replace(new RegExp(`\\$${groupNum}`, 'g'), captureValue || '');
          }
        } else {
        ...
        contentStr = contentStr.substring(0, fullStart) + replacement + contentStr.substring(fullEnd);
      }
    }
    return contentStr;
  }
...
export function validateOptions(options: unknown): options is QueryParamTransformerOptions {
  ...
  if (typeof opts.paramName !== 'string') return false;

  if (!opts.values && !opts.valueRange) return false;
  
  if (opts.valueRange) {
    if (typeof opts.valueRange !== 'object' || opts.valueRange === null) return false;
    ...
    if (!Array.isArray(opts.values)) return false;
    for (const val of opts.values) {
      if (typeof val !== 'string') return false;
    }
  }
  
  if (!Array.isArray(opts.replacements)) return false;
  for (const rule of opts.replacements) {
    if (typeof rule !== 'object' || rule === null) return false;
    const r = rule as Record<string, unknown>;
    if (typeof r.pattern !== 'string') return false;
    if (r.captureGroups !== undefined) {
      if (!Array.isArray(r.captureGroups)) return false;
      for (const group of r.captureGroups) {
        if (typeof group !== 'number') return false;
      }
    }
    if (r.replacement !== undefined && typeof r.replacement !== 'string') return false;
  }
