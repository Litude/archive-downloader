...
export function resolveCommonCrawlRevisitRecords(files: { file: CommonCrawlDownloadedFile, entry: ExtendedCdxEntry }[]): void {
  for (const { file, entry } of files) {
    if (entry.mimetype === "warc/revisit") {
      ...
      if (warcType !== "revisit") {
        throw new Error(`Expected WARC-Type header with value "revisit" for revisit record, but got "${warcType}" for ${entry.url} (${entry.timestamp})`);
      }
      const refersToUrl = file.metadata?.find(([k, _]) => k.toLowerCase() === "warc-refers-to-target-uri")?.[1];
      const refersToDate = file.metadata?.find(([k, _]) => k.toLowerCase() === "warc-refers-to-date")?.[1];
      if (!refersToUrl || !refersToDate) {
        throw new Error(`Missing WARC-Refers-To-Target-URI or WARC-Refers-To-Date header in revisit record for ${entry.url} (${entry.timestamp})`);
      }
      const refersToTimestamp = DateTime.fromISO(refersToDate).toUTC().toFormat("yyyyMMddHHmmss");
      const referredEntry = files.find(
        (f) =>
          f.entry.url === refersToUrl &&
          f.entry.timestamp === refersToTimestamp,
      );
      if (!referredEntry) {
        throw new Error(`Could not find referred record for revisit record ${entry.url} (${entry.timestamp}). Referred URL: ${refersToUrl}, Referred Date: ${refersToDate}`);
      }
      console.log(`Resolving revisit record ${entry.url} (${entry.timestamp}) to referred record ${referredEntry.entry.url} (${referredEntry.entry.timestamp})`);
