...
enum DuplicateUrlSlashParsingMode {
...
// This assumes url1 and url2 have the same urlkey already, so Lax does not actually check anything
function isUrlSlashMatch(url1: string, url2: string, mode: DuplicateUrlSlashParsingMode): boolean {
  if (mode === DuplicateUrlSlashParsingMode.Strict) {
    const urlObj1 = new URL(url1);
    const urlObj2 = new URL(url2);
    return urlObj1.pathname.toLowerCase() === urlObj2.pathname.toLowerCase();
  } else if (mode === DuplicateUrlSlashParsingMode.StrictWithValid) {
    const urlObj1 = new URL(url1);
    const urlObj2 = new URL(url2);
...
function handleDuplicateCapture({
  ...
  requestUrl: string;
  duplicateUrlParsingMode: DuplicateUrlSlashParsingMode;
  response?: {
    statusCode: number;
    originalUrl: string;
  };
  ...
  const filteredSlashModeEntryCount = potentialDuplicates.filter(
    (entry) => !isUrlSlashMatch(entry.url, requestUrl, duplicateUrlParsingMode),
  ).length;
  ...
  if (response) {
    ...
    finalMatchingSnapShot = isUrlSlashMatch(
  ...
  const unavailableOtherUniqueEntries = groupedNonMatchingEntryKeys
    ...
    .filter(isDefined)
    .filter((entry) => isUrlSlashMatch(entry.url, requestUrl, duplicateUrlParsingMode));
...
async function resolveDuplicateSnapshots({
  ...
  requestUrl: string;
  slashMode: DuplicateUrlSlashParsingMode;
  ...
  for (const [timestamp, snapshotsAtTimestamp] of snapshotsByTimestamp) {
    if (snapshotsAtTimestamp.length === 1) {
      ...
      if (limitedCaptures.length > 0) {
        ...
        if (inLimitedCapture) {
          const slashNonMatchCount = snapshotsAtTimestamp.filter(
            (s) => !isUrlSlashMatch(s.url, requestUrl, slashMode),
          ).length;
      ...
      if (allSameStatusAndUrl) {
        ...
        if (isUrlSlashMatch(snapshot.url, requestUrl, slashMode)) {
  ...
  if (slashModeMismatchFiltered > 0) {
    console.log(
      `Filtered out ${slashModeMismatchFiltered} snapshots that did not match the slash mode.`,
    );
  }
